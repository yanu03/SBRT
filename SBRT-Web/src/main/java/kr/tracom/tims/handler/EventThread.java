...
public class EventThread extends Thread {
	...
	/*
	 * private boolean checkChangeRunTypeBusOperInfo(AtBusInfo busInfo) { String
	 * impId = busInfo.getImpId(); if ((impId != null) && (impId.isEmpty() ==
	 * false)) { AtBusInfo oldBusInfo = (AtBusInfo) g_busOperInfoMap.get(impId); if
	 * (oldBusInfo == null || oldBusInfo.getRunType() != busInfo.getRunType()) { if
	 * (busInfo != null && oldBusInfo != null) {
	 * logger.info("checkChangeRunTypeBusInfo busInfo: {}, oldBusInfo: {}", busInfo,
	 * oldBusInfo); } if(g_busOperInfoMap!=null) { g_busOperInfoMap.put(impId,
	 * busInfo); } else{ g_busOperInfoMap = new HashMap<>();
	 * g_busOperInfoMap.put(impId, busInfo); } return true; } return false; } return
	 * false; }
	 */
			...
			for (int i = 0; i < request.getAttrCount(); i++) {
					...
					try {
						...
						try {
							Map<String, Object> curAllocPlInfo = curInfoMapper
									.selectCurAllocPlInfoByOperVhcId(busInfoMap);
							if (curAllocPlInfo == null || CommonUtil.empty(curAllocPlInfo.get("OPER_VHC_ID"))) {
								...
								busInfoMap.put("ALLOC_NO", curAllocPlInfo.get("ALLOC_NO"));
								busInfoMap.put("OPER_SN", curAllocPlInfo.get("OPER_SN"));
								if (busInfoMap.get("REP_ROUT_ID") == null
						 ...
						 * wsDataMap.put("ROUT_ID", busInfoMap.get("ROUT_ID")); wsDataMap.put("ROUT_NM",
						 * busInfoMap.get("ROUT_NM")); wsDataMap.put("VHC_NO",
						 * busInfoMap.get("BUS_NO"));
						 * 
						 * wsDataMap.put("CUR_SPD", busInfoMap.get("SPEED")); wsDataMap.put("VHC_ID",
						 * vhcInfo.get("VHC_ID")); wsDataMap.put("DVC_ID", vhcInfo.get("DVC_ID"));
						 * wsDataMap.put("GPS_X", busInfoMap.get("LONGITUDE")); wsDataMap.put("GPS_Y",
						 * busInfoMap.get("LATITUDE")); wsDataMap.put("PREV_NODE_NM",
						 * busInfoMap.get("PREV_NODE_NM")); // 이전 정류소/교차로 wsDataMap.put("NEXT_NODE_ID",
						 * busInfoMap.get("NEXT_NODE_ID")); // 다음 정류소/교차로 wsDataMap.put("NEXT_NODE_NM",
						 * busInfoMap.get("NEXT_NODE_NM")); wsDataMap.put("NEXT_NODE_TYPE",
						 * busInfoMap.get("NEXT_NODE_TYPE")); wsDataMap.put("OPER_STS",
						 * busInfoMap.get("OPER_STS")); wsDataMap.put("LINK_ID",
						 * busInfoMap.get("LINK_ID")); wsDataMap.put("NODE_SN",
						 * busInfoMap.get("NODE_SN"));
						 */
					} catch (Exception e) {
						logger.error("Exception {}", e);
					}
				...
				case BrtAtCode.BUS_OPER_EVENT: // 운행 이벤트 정보
					...
					try {
						...
						try {
							...
							if (curAllocPlInfo == null) {
								...
								if (checkLastCourse(busEventMap) == false
										&& (eventCode == (byte) 0x03 || eventCode == (byte) 0x04
												|| (eventCode == (byte) 0x01 || eventCode == (byte) 0x02)
														&& routMap2.get("ED_STTN_ID").equals(eventData))) // 기점, 종점을 잡지
																											// 못했을때
								{
								...
								if (checkLastCourse(busEventMap) == false && // 해당 노선이 코스의 마지막이 아닐때
									...
									if (CommonUtil.notEmpty(curNearStr)) {
										...
										sbrtRouteInfo.setUpdateTm(new AtTimeStamp(LocalDateTime.now()
												.format(DateTimeFormatter.ofPattern(PlatformConfig.PLF_DT_FORMAT))));
										...
										if (("IMP0010001".equals(sessionId) == false)
												&& ("IMP0010002".equals(sessionId) == false)
												&& ("IMP0010010".equals(sessionId) == false)
												&& ("IMP0010011".equals(sessionId) == false)
												&& ("IMP0010012".equals(sessionId) == false)) {
											logger.debug("kafkaProducer.sendKafka before sbrtRouteInfo {}",
													sbrtRouteInfo);
											kafkaProducer.sendKafka(setRequest, sessionId);
										}
							...
							if (CommonUtil.empty(busEventMap.get("ALLOC_NO")) == false
										...
										|| eventCode == 0x05 || eventCode == 0x06) // 종점점 출/도착 인 경우
								{
									 ...
									 * sttnEventMap.put("NODE_ID", busEvent.getEventData()); // 정류장 출/도착인 경우
									 * EVENT_DATA를 사용 setOperEventData(sttnEventMap);
									 * 
									 * busEventMap.put("NODE_NM", sttnEventMap.get("NODE_NM")); // 출/도착 정류소명
									 * busEventMap.put("NODE_TYPE", sttnEventMap.get("NODE_TYPE"));
									 * busEventMap.put("PREV_NODE_NM", sttnEventMap.get("PREV_NODE_NM")); // 이전
									 * 정류소/교차로 busEventMap.put("NEXT_NODE_ID", sttnEventMap.get("NEXT_NODE_ID")); //
									 * 다음 정류소/교차로 busEventMap.put("NEXT_NODE_NM", sttnEventMap.get("NEXT_NODE_NM"));
									 * busEventMap.put("NEXT_NODE_TYPE", sttnEventMap.get("NEXT_NODE_TYPE"));
									 * 
									 * busEventMap.put("CUR_NODE_TYPE", sttnEventMap.get("CUR_NODE_TYPE")); // 현재
									 * 정류소/교차로 busEventMap.put("CUR_NODE_ID", sttnEventMap.get("CUR_NODE_ID"));
						...
						/*
						 * wsDataMap = new HashMap<>(); wsDataMap.put("ATTR_ID", attrId);
						 * wsDataMap.put("VHC_NO", busEvent.getBusNo()); wsDataMap.put("ROUT_ID",
						 * busEvent.getRouteId()); wsDataMap.put("ROUT_NM", busEventMap.get("ROUT_NM"));
						 * wsDataMap.put("VHC_ID", vhcInfo.get("VHC_ID")); wsDataMap.put("DVC_ID",
						 * vhcInfo.get("DVC_ID")); wsDataMap.put("GPS_X", busEventMap.get("LONGITUDE"));
...

						{
							 ...
							 * 
							 * wsDataMap.put("CUR_NODE_TYPE", busEventMap.get("CUR_NODE_TYPE")); // 다음
							 * 정류소/교차로 wsDataMap.put("CUR_NODE_ID", busEventMap.get("CUR_NODE_ID"));
						...
						/*
						 * wsDataMap.put("EVT_CODE", eventCd); wsDataMap.put("EVT_TYPE", eventCdName);
						 * wsDataMap.put("CUR_SPD", busEventMap.get("SPEED")); wsDataMap.put("EVT_DATA",
						 * busEventMap.get("EVENT_DATA")); wsDataMap.put("NODE_SN",
						 * busEventMap.get("NODE_SN"));
						 */
					} catch (Exception e) {
						logger.error("Exception {}", e);
					}
