...
public class ST0701Service extends ServiceSupport {
	...
	public List ST0701G1R0() throws Exception {
		...
		String temp[] = map.get("CPLNT_DIV").toString().replace("[","").replace("]","").replace(" ","").split(",");
		map.put("CPLNT_DIV", temp);
		return st0701Mapper.ST0701G1R0(map);
	}
	
	/*public List ST0701G1R1() throws Exception {
		Map<String, Object> map = getSimpleDataMap("dma_sub_search");
		String temp[] = map.get("ROUT_ID").toString().replace("[","").replace("]","").replace(" ","").split(",");
		map.put("ROUT_ID", temp);
		return st0701Mapper.ST0701G1R1(map);
	}
	
	public List ST0701G1R2() throws Exception {
		Map<String, Object> map = getSimpleDataMap("dma_sub_search");
		String temp[] = map.get("ROUT_ID").toString().replace("[","").replace("]","").replace(" ","").split(",");
		map.put("ROUT_ID", temp);
		return st0701Mapper.ST0701G1R2(map);
	}*/
