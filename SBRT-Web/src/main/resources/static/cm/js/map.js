...
var routMap = {
  ...
  vhcInfo : [],
	...
	//================================지도 이동 관련===============================================
	FOCUS_MODE : "Y", //차량이 움직일때마다 지도의 중심이 해당 버스를 따라갈지 여부(Y, N)
	//================================지도 이동 관련===============================================
	...
	ZINDEX_BUS_OVERLAY : 5, //버스 오버레이 z-index
...
routMap.initMap = function(mapId,options) {
	...
	kakao.maps.event.addListener(map, 'zoom_changed', function() {        
	    ...
	    console.log(routMap.mapInfo[mapId].polylines)
...
routMap.getVhcInfo = function(vhcId){
  if(typeof routMap.vhcInfo[vhcId] !== "undefined" || routMap.vhcInfo[vhcId] != null) {
    return routMap.vhcInfo[vhcId];
  }
  else {
    routMap.vhcInfo[vhcId] = new VhcMapInfo();
  }
}
...
routMap.addMarkerInter = function(mapId, data, grid, idx, focusIdx, isOverLayHidden) {
	...
	if(data.NODE_TYPE == routMap.NODE_TYPE.CROSS){
		zIndex = routMap.ZINDEX_CRS_MARKER;
		...
		zIndex = routMap.ZINDEX_STTN_MARKER;
...
//		var msg = "<div class = 'customoverlay'>"
		...
		overlay = new kakao.maps.CustomOverlay({
			...
			zIndex : routMap.ZINDEX_STTN_OVERLAY
		});
...
routMap.addSoundMarkerInter = function(mapId, data, grid, idx, focusIdx) {
	...
	var marker = null;
	if(idx==focusIdx) {
		zIndex = routMap.ZINDEX_FOCUS_MARKER;
		// 마커 이미지를 생성합니다    
		marker = new kakao.maps.Marker({
			position : new kakao.maps.LatLng(data.GPS_Y, data.GPS_X), // Marker의 중심좌표 // 설정.
			//title : data.label, // Marker의 라벨.
...
routMap.showBusMarker = function(mapId, data, idx, focusIdx, busGrid) {
	...
	var zIndex= routMap.ZINDEX_BUS_MARKER;
	//확인이 필요함
	if(data.VHC_NO.includes("우진")){
	...
	if(data.OCR_DTM != "") {
		...
		//메모리 문제 발생 안하는지 확인필요
		var today = new Date();
		var ymd = today.getFullYear() + '-' + leadingZeros(today.getMonth()+1, 2) + '-' + leadingZeros(today.getDate(), 2);
		var ocrDtm = ymd + " " + data.OCR_DTM;
		var ocrDtmDate = new Date(ocrDtm);
		
		var secDiffer = (today - ocrDtmDate) / 1000;
		
		if(secDiffer >= routMap.RECEIVE_WAIT_TIME) {
			markerImage = new kakao.maps.MarkerImage("/cm/images/tmap/bus_gray.png", imageSize);
			markerSelImage = new kakao.maps.MarkerImage("/cm/images/tmap/bus_gray_select.png", imageSize);
		}		
	}
	
	var marker = null;
	if(idx==focusIdx) {
		zIndex = routMap.ZINDEX_FOCUS_BUS_MARKER;
		// 마커 이미지를 생성합니다    
		marker = new kakao.maps.Marker({
			position : new kakao.maps.LatLng(data.GPS_Y, data.GPS_X), // Marker의 중심좌표 // 설정.
			//title : data.label, // Marker의 라벨.
...
//		routMap.mapInfo[mapId].busOverArr[idx].setPosition(position2);
		...
		rightClickMsg += '	            <span class="text">버스 상태정보</span>'
		rightClickMsg += '     </a>'
		...
		rightClickOverlay = new kakao.maps.CustomOverlay({
			...
			position: marker.getPosition(),
			zIndex : routMap.ZINDEX_BUS_RIGHTCLICK_OVERLAY
		});		
...
routMap.showBubbleOverlay = function(mapId, data, marker, idx, focusIdx) {
	...
	if (typeof data.VHC_ID == "undefined") {
		if(data.NODE_TYPE == routMap.NODE_TYPE.BUSSTOP){
			...
			zIndex = routMap.ZINDEX_CRS_OVERLAY;
			msg = "<div class = 'customoverlay cross'>";
	...
	//버스
		...
		zIndex = routMap.ZINDEX_BUS_MARKER;
		overlayName = "번호없음";
		if (typeof data.VHC_NO != "undefined") {
			overlayName = data.VHC_NO
		} else if (typeof data.RPC_VHC_NO != "undefined") {
	...
	if(typeof data.VHC_ID == "undefined") {
		...
		else if(routMap.SHOW_CRS_OVERLAY == "Y" && data.NODE_TYPE == routMap.NODE_TYPE.CROSS) {
...
routMap.showClickBusOverlay = function(mapId, data, idx, focusIdx, marker) {
	var zIndex = routMap.ZINDEX_BUS_OVERLAY;
...
routMap.showDsptchOverlay = function(mapId, data, idx, focusIdx, marker) {
	if(routMap.SHOW_DISPATCH_OVERLAY == "N") {
  ...
  if(routMap.mapInfo[mapId].dsptchOverArr != null) {
	...
	var zIndex = routMap.ZINDEX_DISPATCH_OVERLAY;
	var showMessage = "";
	var min = "";
	var sec = "";
	var dsptchMessage = "";
...
/*			if(parseInt(data.MESSAGE) > 0) {
	...
	if(routMap.mapInfo[mapId].dsptchOverArr != null && routMap.mapInfo[mapId].eventOverArr != null) {
		//if(routMap.mapInfo[mapId].dsptchOverArr.length != 0 && routMap.mapInfo[mapId].eventOverArr.length == 0) {
		if(routMap.mapInfo[mapId].dsptchOverArr[0] != null && routMap.mapInfo[mapId].eventOverArr[0] == null) {
			setTimeout(function() {
...
routMap.showEventOverlay = function(mapId, data, idx, focusIdx, marker) {
	if(routMap.SHOW_EVENT_OVERLAY == "N") {
  ...
  */
	...
	var zIndex = routMap.ZINDEX_EVENT_OVERLAY;
	var eventMsg = "";
	...
	//출발이벤트 or 차량 속력이 30km/h 이상일 때 이벤트 오버레이 삭제
		...
		/*	if(routMap.mapInfo[mapId].divEvent == "ET001"){
			...
			setTimeout(function() {
				//if(routMap.mapInfo[mapId].dsptchOverArr.length != 0) {
				if(routMap.mapInfo[mapId].dsptchOverArr != null) {
					if(routMap.mapInfo[mapId].dsptchOverArr[0] != 0) {
						//$("#busInfo-closer").css("bottom", "42px");
...
routMap.showSigMarker = function(mapId, baseData, socketData) {
	...
	if(routMap.mapInfo[mapId].isShowCrs == "on"){
		...
		if(socketData != null && typeof socketData != "undefined"){
			//var phaseNoArr = baseData.PHASE_NO.indexOf(socketData.PHASE_NO)
			...
			//	}
			//}
      if(baseData.PHASE_NO.indexOf(socketData.PHASE_NO)>-1){
...
routMap.showOnlyMarker = function(mapId, data, idx, focusIdx, grid) {
	...
	if(idx==focusIdx) {
		zIndex = routMap.ZINDEX_FOCUS_STTNCRS_MARKER;
		// 마커 이미지를 생성합니다    
		marker = new kakao.maps.Marker({
			position : new kakao.maps.LatLng(data.GPS_Y, data.GPS_X), // Marker의 중심좌표 // 설정.
			//title : data.label, // Marker의 라벨.
